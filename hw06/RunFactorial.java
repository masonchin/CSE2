...
public class RunFactorial {
    ...
    //prompts user to enter a number between 9 and 16
        ...
        int number;             //will store input
    
        if (myScanner.hasNextInt()) {           //if input is an integer, input will be stored in variable number
    ...
    //while the count is less than or equal to inputted number, count will be incremented and will be multiplied by variable factorial
        while (number >= count) {
            ...
            factorial *= count;         //multiplies factorial by count
